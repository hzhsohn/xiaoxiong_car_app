...
- (void)viewDidLoad
{
    ...
    [lbStatus setText:NSLocalizedString(@"mjpeg_lbStatus", nil)];
    [lbStatusText2 setText:NSLocalizedString(@"mjpeg_lbStatusText2_ph", nil)];
    m_dwDataCulTime=0;
    ...
    LOCK_CS(&m_cs);
    //在这里进行处理
    m_Mjpg=(TagMjpg_Stream*)malloc(sizeof(TagMjpg_Stream));
    mjpg_init(m_Mjpg);
    //begin connect
    m_bConnecting=false;
...
-(void)connectFail
{
    [lbStatusText setText:NSLocalizedString(@"mjpeg_connectFail",nil)];
}
-(void)connectOK
{
    [lbStatusText setText:NSLocalizedString(@"mjpeg_connectOK",nil)];
}
...
-(void)mjpgConnectThread
{
    @autoreleasepool {
        ...
        if(m_Mjpg)
        {
            ...
            {
                if(mjpg_connect(m_Mjpg,ip,m_pHostInfo->port,m_pHostInfo->username,m_pHostInfo->password))
                {
                    [self connectFail];
                }
                else
...
-(void)ConnectMjpeg
{
    if (m_bDisconnected) {
        if (m_bConnecting==false) {
            m_bConnecting=true;
            m_dwDataCul=0;
            [lbStatusText setText:NSLocalizedString(@"mjpeg_connecting",nil)];
            NSThread* thread;
            thread = [[NSThread alloc] initWithTarget:self selector:@selector(mjpgConnectThread) object:nil];
...
-(void)mjpg_thread:(TagMjpg_Stream *)mjpg
{
	...
	if(nRet>0)
	{
        ...
        {
            [self mjpg_stream_filter:mjpg:szTmp:nRet];
        }
        
	}
    else if(nRet==0){
        if (m_getBegin==1) {
            mjpg_get_stream(m_Mjpg);
            m_getBegin=2;
        }
    }
    else if(nRet==-1)
    {
        //用户名和密码有错误，不再连接
            ...
            if (m_bConnecting==false) {
                ...
                {
                    lbStatusText.text=NSLocalizedString(@"mjpeg_verify_err",nil) ;
                }
                else
                {
                    if (false==m_bDisconnected) {
                    lbStatusText.text=[NSString stringWithFormat:NSLocalizedString(@"mjpeg_disconnect",nil),3] ;
    ...
    if (time(NULL)-m_dwDataCulTime>1000) {
        ...
        [lbStatusText2 setText:[NSString stringWithFormat:NSLocalizedString(@"mjpeg_lbStatusText2", nil),
