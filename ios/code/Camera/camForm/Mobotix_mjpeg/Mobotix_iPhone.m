...
- (void)viewDidLoad
{
    ...
    [lbStatus setText:NSLocalizedString(@"mobotix_lbStatus", nil)];
    [lbStatusText2 setText:NSLocalizedString(@"mobotix_lbStatusText2_ph", nil)];
    m_dwDataCulTime=0;
    ...
    LOCK_CS(&m_cs);
    //在这里进行处理
    m_Mjpg=(TagMjpg_Mobotix*)malloc(sizeof(TagMjpg_Mobotix));
    mobotix_init(m_Mjpg);
    //begin connect
    m_bConnecting=false;
...
-(void)connectFail
{
    [lbStatusText setText:NSLocalizedString(@"mobotix_connectFail",nil)];
}
-(void)connectOK
{
    [lbStatusText setText:NSLocalizedString(@"mobotix_connectOK",nil)];
}
...
-(void)mobotixConnectThread
{
    @autoreleasepool {
        ...
        if(m_Mjpg)
        {
            ...
            {
                if(mobotix_connect(m_Mjpg,ip,m_pHostInfo->port,m_pHostInfo->username,m_pHostInfo->password))
                {
                    [self connectFail];
                }
                else
...
-(void)ConnectMjpeg
{
    if (m_bDisconnected) {
        if (m_bConnecting==false) {
            m_bConnecting=true;
            m_dwDataCul=0;
            [lbStatusText setText:NSLocalizedString(@"mobotix_connecting",nil)];
            NSThread* thread;
            thread = [[NSThread alloc] initWithTarget:self selector:@selector(mobotixConnectThread) object:nil];
...
-(void)mobotix_thread:(TagMjpg_Mobotix *)mjpg
{
	...
	if(nRet>0)
	{
        if(mobotix_check_authorized(mjpg,szTmp,nRet))
        {
            [self mobotix_stream_filter:mjpg:szTmp:nRet];
        }
        
	}
    else if(nRet==0){
        if (m_getBegin==1) {
            mobotix_get_stream(m_Mjpg);
            m_getBegin=2;
        }
    }
    else if(nRet==-1)
    {
        ...
        if (m_bConnecting==false) {
            ...
            {
                lbStatusText.text=NSLocalizedString(@"mobotix_verify_err",nil) ;
            }
            else
            {
                if (false==m_bDisconnected) {
                    lbStatusText.text=[NSString stringWithFormat:NSLocalizedString(@"mobotix_disconnect",nil),3] ;
    ...
    if (time(NULL)-m_dwDataCulTime>1000) {
        ...
        [lbStatusText2 setText:[NSString stringWithFormat:NSLocalizedString(@"mobotix_lbStatusText2", nil), (float)(m_dwDataCul/1024.0f)]];
