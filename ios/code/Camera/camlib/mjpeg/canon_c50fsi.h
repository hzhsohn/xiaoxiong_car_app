/**********************************
 ...
 this is canon vb-c50fsi module
 
**********************************/

#pragma once
#ifndef _ZH_CANON_C50FSI_STREAM_
#ifdef __cplusplus
extern "C"{
#endif
...
#define CAMERA_PAGE_C50FSI      "/-wvhttp-01-/getoneshot?camera_id=1&frame_count=no_limit"
    
#define MJPG_JPG_SIZE           800*600*3

typedef struct _TagMjpg_C50FSI{
	SOCKET s;
...
static bool c50fsi_verify_connect(TagMjpg_C50FSI *mjpg)
{
...
//验证授权
static int c50fsi_check_authorized(TagMjpg_C50FSI *mjpg,char*buf,int len)
{
    char a[20];
    char b[10];
    char c[50];
    ...
    if(mjpg->bAuthorized==0)
    {
        ...
        if(0==strcmpi("401",b) || 0==strcmpi("Unauthorized",c))
        {
            ...
            sprintf(sendbuf,buf,CAMERA_PAGE_C50FSI,base);
...
static void c50fsi_stream_filter(TagMjpg_C50FSI *mjpg,char *btBuf,int nSize)
{
	int nCucl=0;
...
/* 循环执行 */
static void c50fsi_thread(TagMjpg_C50FSI *mjpg)
{
	int nRet;
	char szTmp[65535];
	...
	if(nRet>0)
	{
        ...
        {
			c50fsi_stream_filter(mjpg,szTmp,nRet);
        }
	}
}
/* 执行:2 */
static int c50fsi_connect(TagMjpg_C50FSI *mjpg,char *szIp,unsigned short wPort,const char *user,const char *pwd)
{
    mjpg->nCache=0;
	mjpg->nReadPos=0;
	mjpg->isget=0;
...
/* 执行:3 */
static void c50fsi_get_stream(TagMjpg_C50FSI *mjpg)
{
    char sendBuf[256];
...
/* 执行:1 */
static void c50fsi_init(TagMjpg_C50FSI *mjpg)
{
    mjpg->bVerifyNum=0;
}

/* 退出时执行 */
static void c50fsi_close(TagMjpg_C50FSI *mjpg)
{
    dhsClose(mjpg->s);
}
...
#endif
#define _ZH_CANON_C50FSI_STREAM_
#endif
