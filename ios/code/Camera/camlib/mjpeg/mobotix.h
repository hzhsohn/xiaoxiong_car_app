/*********************************
 ...
 this is mobotix module
 
*********************************/

#pragma once
#ifndef _ZH_MOBOTIX_STREAM_
#ifdef __cplusplus
extern "C"{
#endif
...
#define CAMERA_PAGE_MOBOTIX     "/cgi-bin/faststream.jpg?stream=full&fps=0"
    
#define MJPG_JPG_SIZE           800*600*3

typedef struct _TagMjpg_Mobotix{
	SOCKET s;
...
static bool mobotix_verify_connect(TagMjpg_Mobotix *mjpg)
{
...
//验证授权
static int mobotix_check_authorized(TagMjpg_Mobotix *mjpg,char*buf,int len)
{
    char a[20];
    char b[10];
    char c[50];
    ...
    if(mjpg->bAuthorized==0)
    {
        ...
        if(0==strcmpi("401",b) || 0==strcmpi("Unauthorized",c))
        {
            ...
            sprintf(sendbuf,buf,CAMERA_PAGE_MOBOTIX,base);
...
static void mobotix_stream_filter(TagMjpg_Mobotix *mjpg,char *btBuf,int nSize)
{
	int nCucl=0;
...
/* 循环执行 */
static void mobotix_thread(TagMjpg_Mobotix *mjpg)
{
	int nRet;
	char szTmp[65535];
	...
	if(nRet>0)
	{
        ...
        {
			mobotix_stream_filter(mjpg,szTmp,nRet);
        }
	}
}
/* 执行:2 */
static int mobotix_connect(TagMjpg_Mobotix *mjpg,char *szIp,unsigned short wPort,const char *user,const char *pwd)
{
    mjpg->nCache=0;
	mjpg->nReadPos=0;
	mjpg->isget=0;
...
/* 执行:3 */
static void mobotix_get_stream(TagMjpg_Mobotix *mjpg)
{
    char sendBuf[256];
...
/* 执行:1 */
static void mobotix_init(TagMjpg_Mobotix *mjpg)
{
    mjpg->bVerifyNum=0;
}

/* 退出时执行 */
static void mobotix_close(TagMjpg_Mobotix *mjpg)
{
    dhsClose(mjpg->s);
}
