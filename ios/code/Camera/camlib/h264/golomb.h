...
extern const uint8_t zh_ff_golomb_vlc_len[512];
extern const uint8_t zh_ff_ue_golomb_vlc_code[512];
extern const  int8_t zh_ff_se_golomb_vlc_code[512];
...
static inline int get_ue_golomb(GetBitContext *gb){
    ...
    if(buf >= (1<<27)){
        buf >>= 32 - 9;
        LAST_SKIP_BITS(re, gb, zh_ff_golomb_vlc_len[buf]);
        CLOSE_READER(re, gb);

        return zh_ff_ue_golomb_vlc_code[buf];
    }else{
