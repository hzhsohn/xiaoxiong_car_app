...
@interface CAIDModify ()<WebPocDelegate>
{
...
-(void) WebProcCallBackData:(NSURL*)url :(NSData*)data
{
    ...
    if([page isEqualToString:@"caid_mod.i.php"])
    {
        //解释data
        ...
        switch ([[result objectForKey:@"nRet"] intValue]) {
            case 1:
            {
                [ind setAlpha:0];
                [ind setHidden:YES];
                [self.delegate CAIDModifyContent_change:txtContent.text];
...
- (IBAction)btnModify_click:(id)sender
{
    ...
    //更新记录
    NSString* str=[NSString stringWithFormat:@"k=%@&autoid=%@&content=%@",
                   [GlobalParameter getLoginKey],
                   self.modify_autoid,
                   ttCnt];
    [_web sendData:[GlobalParameter getIOTAddrByCAID:@"caid_mod.i.php"] parameter:str];
}
@end
